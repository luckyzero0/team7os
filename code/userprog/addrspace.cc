...
AddrSpace::AddrSpace(OpenFile *theExecutable, int spaceID) : fileTable(MaxOpenFiles) {
	NoffHeader noffH;
	unsigned int i, size;
...
#ifdef USE_TLB
		pageTable[i].spaceID = spaceID;
		pageTable[i].inUse = false; // this is necessary for later when we load a page, we copy from this pageTable to the IPT, and this value should be true always in the pageTable
...
void AddrSpace::AddNewThread(Thread* newThread) {
	...
	for (int i = 0; i < UserStackSize / PageSize; i++) {
		...
		pageTable[startVPN + i].inUse = false;
#endif
	}
